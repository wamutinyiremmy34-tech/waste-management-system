...
// Leaflet/react-leaflet require window — must be loaded client-side only.
// Using Next.js dynamic import with ssr:false is the standard pattern.
const OperationalMap = dynamic(() => import("@/components/OperationalMap"), {
  ssr: false,
  loading: () => (
    <div className="flex h-96 items-center justify-center rounded-lg border border-stone-200 bg-stone-50 text-sm text-stone-400">
      Loading map…
    </div>
  ),
});
...
export default function AdminPage() {
  ...
  const [mapLayerData, setMapLayerData] = useState<Awaited<ReturnType<typeof api.mapData>> | null>(null);
  const [zonesGeoJSON, setZonesGeoJSON] = useState<Awaited<ReturnType<typeof api.zonesGeoJSON>> | null>(null);
  // Map layer toggles
  const [mapShowZones, setMapShowZones] = useState(true);
  const [mapShowBins, setMapShowBins] = useState(true);
  const [mapShowComplaints, setMapShowComplaints] = useState(true);
  const [mapShowPickups, setMapShowPickups] = useState(true);
  const [mapShowHotspots, setMapShowHotspots] = useState(true);
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "map" | "zones" | "priority" | "complaints" | "reports">("overview");
  const tabsLoadedRef = useRef<Record<string, boolean>>({ overview: false });
  ...
  const refreshOverview = useCallback(() => {
    ...
    Promise.all([
    ...
    ]).then(([dash, waste, cStats, opSum, trendData, envData, hots]) => {
      setStats(dash);
      ...
      tabsLoadedRef.current.overview = true;
  ...
  const refreshMap = useCallback(() => {
    ...
    setMapLoading(true);
    Promise.all([
      api.mapData(token).catch(() => null),
      api.zonesGeoJSON(token).catch(() => null),
      api.zonePerformance(token).catch(() => null),
    ]).then(([mapLayers, zoneGeo, zones]) => {
      setMapLayerData(mapLayers);
      setZonesGeoJSON(zoneGeo);
      setZonePerf(zones);
      tabsLoadedRef.current.zones = true;
      tabsLoadedRef.current.map = true;
      tabsLoadedRef.current.priority = true;
      setMapLoading(false);
    });
  }, [token]);
  ...
  useEffect(() => {
    ...
    if (activeTab === "complaints") {
      if (!tabsLoadedRef.current.complaints) {
        refreshComplaints();
        tabsLoadedRef.current.complaints = true;
      }
      return;
    }
    if ((activeTab === "map" || activeTab === "zones" || activeTab === "priority") && !tabsLoadedRef.current.map) {
      refreshMap();
    }
  }, [activeTab, refreshComplaints, refreshMap]);
  ...
  }, [token, activeTab, refreshOverview, refreshComplaints, refreshMap]);
  ...
  useEffect(() => {
    if (!loading && token && user && ADMIN_ROLES.includes(user.role)) {
      refresh();
    }
      ...
      <div className="border-b border-stone-200 bg-white">
        <div className="mx-auto max-w-6xl px-6">
          <nav className="flex gap-1">
            {(["overview", "map", "zones", "priority", "complaints", "reports"] as const).map((tab) => (
              <button
                key={tab}
            ...
            {stats && (
              <section>
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                  ...
                  <StatCard label="Failed collections" value={opSummary?.kpis.failed ?? 0} warn={(opSummary?.kpis.failed ?? 0) > 0} />
                  ...
                  <StatCard
                    ...
                    value={`${stats.total_collections > 0 ? Math.round((stats.completed_collections / stats.total_collections) * 100) : 0}%`}
                    highlight={stats.total_collections > 0 && (stats.completed_collections / stats.total_collections) >= 0.8}
                    warn={stats.total_collections > 0 && (stats.completed_collections / stats.total_collections) < 0.6}
                  />
                </div>
              </section>
            )}
        ...
        {activeTab === "map" && (
          <section>
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              <div>
                <h2 className="text-lg font-semibold text-stone-900">Operational map</h2>
                <p className="text-xs text-stone-400 mt-0.5">
                  Spatial data from PostGIS. Tiles: © OpenStreetMap contributors.
                </p>
              </div>
              {/* Layer toggles */}
              <div className="flex flex-wrap gap-2">
                {[
                  { key: "zones", label: "Zones", val: mapShowZones, set: setMapShowZones, colour: "#1b4332" },
                  { key: "complaints", label: "Complaints", val: mapShowComplaints, set: setMapShowComplaints, colour: "#7c3aed" },
                  { key: "bins", label: "Bins", val: mapShowBins, set: setMapShowBins, colour: "#2563eb" },
                  { key: "pickups", label: "Pickups", val: mapShowPickups, set: setMapShowPickups, colour: "#6b7280" },
                  { key: "hotspots", label: "Hotspots", val: mapShowHotspots, set: setMapShowHotspots, colour: "#b45309" },
                ].map(({ key, label, val, set, colour }) => (
                  <button
                    key={key}
                    onClick={() => set(!val)}
                    className={`flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium transition-colors ${
                      val ? "text-white" : "bg-stone-100 text-stone-500"
                    }`}
                    style={val ? { backgroundColor: colour } : undefined}
                  >
                    {label}
                  </button>
                ))}
            ...
            <div className="mb-3 flex flex-wrap gap-4 text-xs text-stone-500">
              <span className="flex items-center gap-1"><span className="inline-block h-2 w-2 rounded-sm bg-[#7c3aed]" /> Complaint</span>
              <span className="flex items-center gap-1"><span className="inline-block h-2 w-3 rounded-sm bg-[#16a34a]" /> Bin (low fill)</span>
              <span className="flex items-center gap-1"><span className="inline-block h-2 w-3 rounded-sm bg-[#dc2626]" /> Bin (full)</span>
              <span className="flex items-center gap-1"><span className="inline-block h-2 w-2 rotate-45 bg-[#6b7280]" /> Pickup</span>
              <span className="flex items-center gap-1"><span className="inline-block h-3 w-3 rounded-full bg-[#fbbf24] opacity-60 border border-[#b45309]" /> Hotspot cluster</span>
            </div>

            <OperationalMap
              complaints={mapShowComplaints ? (mapLayerData?.complaints ?? []) : []}
              bins={mapShowBins ? (mapLayerData?.bins ?? []) : []}
              pickups={mapShowPickups ? (mapLayerData?.pickups ?? []) : []}
              hotspots={mapShowHotspots ? (mapLayerData?.hotspots ?? hotspots) : []}
              zones={mapShowZones ? zonesGeoJSON : null}
              showZones={mapShowZones}
              showBins={mapShowBins}
              showComplaints={mapShowComplaints}
              showPickups={mapShowPickups}
              showHotspots={mapShowHotspots}
              height="560px"
            />

            <div className="mt-3 grid grid-cols-2 gap-3 text-center text-xs sm:grid-cols-4">
              <div className="rounded-lg border border-stone-200 bg-white p-3">
                <p className="font-semibold text-stone-900">{mapLayerData?.complaints.length ?? 0}</p>
                <p className="text-stone-500">Unresolved complaints</p>
              </div>
              <div className="rounded-lg border border-stone-200 bg-white p-3">
                <p className="font-semibold text-stone-900">{mapLayerData?.bins.length ?? 0}</p>
                <p className="text-stone-500">Bins on map</p>
              </div>
              <div className="rounded-lg border border-stone-200 bg-white p-3">
                <p className="font-semibold text-stone-900">{mapLayerData?.pickups.length ?? 0}</p>
                <p className="text-stone-500">Active pickups</p>
              </div>
              <div className="rounded-lg border border-stone-200 bg-white p-3">
                <p className="font-semibold text-stone-900">{(mapLayerData?.hotspots ?? hotspots).length}</p>
                <p className="text-stone-500">Hotspot clusters</p>
              </div>
            </div>
          </section>
        )}
          ...
          <section>
            ...
            <div className="space-y-2">
              {(opSummary?.overdue_pickups ?? []).map((p) => (
                <div key={p.pickup_id} className="rounded-lg border border-stone-200 bg-white p-4">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      {"score_breakdown" in p && p.score_breakdown !== null && typeof p.score_breakdown === "object" && (
                        <div className="mt-2 rounded bg-stone-50 px-3 py-2 text-xs text-stone-600">
                          <p className="font-medium mb-1">Score breakdown:</p>
                          {"overdue_contribution" in p.score_breakdown && (
                            <p>Overdue: {(p.score_breakdown as { overdue_contribution: number }).overdue_contribution} pts</p>
                          )}
                          {"bin_fill_percent" in p.score_breakdown && "bin_contribution" in p.score_breakdown && (
                            <p>Bin fill: {(p.score_breakdown as { bin_fill_percent: number; bin_contribution: number }).bin_fill_percent}% → {(p.score_breakdown as { bin_contribution: number }).bin_contribution} pts</p>
                          )}
                          {"nearby_complaint_count" in p.score_breakdown && "complaint_contribution" in p.score_breakdown && (
                            <p>Nearby complaints: {(p.score_breakdown as { nearby_complaint_count: number; complaint_contribution: number }).nearby_complaint_count} → {(p.score_breakdown as { complaint_contribution: number }).complaint_contribution} pts</p>
                          )}
                        </div>
                      )}
                    </div>
...
// ---------------------------------------------------------------------------
// Reusable stat card — local to admin page (shared StatCard from ui/ used elsewhere)
// ---------------------------------------------------------------------------
function StatCard({
  label,
  value,
